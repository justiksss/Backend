...
class JobsDal:
    ...
    async def get_job_by_uuid(self, uuid: UUID) -> Jobs:
        ...
        if row is not None:
            ...
            jobs_extend = instance_dict(row)
            job = Jobs(id_job=jobs_extend["id_job"], name=jobs_extend["name"], logo=jobs_extend["logo"],
                           location="Prague", company_name=jobs_extend["company_name"],job_type=jobs_extend["job_type"],
                       posted_days_ago=jobs_extend["posted_days_ago"],description=jobs_extend["description"])
